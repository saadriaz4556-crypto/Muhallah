...
class _LocalVibesScreenState extends State<LocalVibesScreen> {
  ...
  void _deletePost(String postId, String? imageUrl) async {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete this post?', style: TextStyle(color: lightText)),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Post deleted successfully'), backgroundColor: statusRed),
                );
              }
            },
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Column(
          ...
          children: [
            Text('Local Vibes 🔥', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
            Text('Posts 24 ghante mein gayab ho jate hain', style: TextStyle(color: Colors.white54, fontSize: 11)),
          ],
        ),
        ...
        actions: [
          TextButton(
            ...
            child: const Text('+ Post', style: TextStyle(color: accentTeal, fontWeight: FontWeight.bold, fontSize: 16)),
          ),
        ],
      ),
        ...
        builder: (context, snapshot) {
            ...
            return const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 16),
                  Text('Abhi koi vibe nahi!\nPehli vibe aap dalo 🔥', textAlign: TextAlign.center, style: TextStyle(color: Colors.white70, fontSize: 18)),
                ],
              ),
            );
          }
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final expiresAt = (post['expires_at'] as dynamic)?.toDate() ?? DateTime.now();
              ...
              return Container(
                ...
                child: Column(
                  ...
                  children: [
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                const Text('Resident', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                                Text(
                                  '${diff.inHours} hours left!',
                                  style: TextStyle(color: timerColor, fontSize: 12, fontWeight: FontWeight.bold),
                                ),
                              ],
                            ),
                          ),
                              ...
                              if (post['user_id'] == _service.currentUserId)
                                ...
                                const PopupMenuItem(
                                  ...
                                  child: Text('Report', style: TextStyle(color: Colors.redAccent)),
                                        ...
                                        // Content
                    if (post['content_text'] != null && post['content_text'].toString().trim().isNotEmpty)
                      Padding(
                    ...
                    if (post['post_type'] == 'photo' && post['media_url'] != null)
                      Padding(
                    ...
                    StreamBuilder<DocumentSnapshot>(
                      ...
                      builder: (context, rxSnapshot) {
                        final hasReacted = rxSnapshot.hasData && rxSnapshot.data!.exists;
                        final rxType = hasReacted ? rxSnapshot.data!['reaction_type'] : null;
                          ...
                          child: Row(
                            children: [
                              _buildAction(
                                ...
                                onTap: () => _service.toggleReaction(postId, 'like'),
                              ),
                              const SizedBox(width: 16),
                              _buildAction(
                                icon: hasHaha ? Icons.sentiment_very_satisfied : Icons.sentiment_satisfied_alt,
                                count: post['haha_count'] ?? 0,
                                color: Colors.amber,
                                onTap: () => _service.toggleReaction(postId, 'haha'),
                              ),
                              const Spacer(),
                              _buildAction(
  ...
  Widget _buildAction({required IconData icon, required int count, required Color color, required VoidCallback onTap}) {
    return InkWell(
      onTap: onTap,
      child: Row(
        children: [
          ...
          Text('$count', style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
