...
class EditInvitationSheet extends StatefulWidget {
  final String docId;
  final Map<String, dynamic> data;

  const EditInvitationSheet({
    super.key,
    required this.docId,
    required this.data,
  });
  ...
  @override
  State<EditInvitationSheet> createState() => _EditInvitationSheetState();
}

class _EditInvitationSheetState extends State<EditInvitationSheet> {
  ...
  void initState() {
    super.initState();
    _selectedCategory = widget.data['category'] ?? 'wedding';
    _currentPhotoUrl = widget.data['coverImageUrl'] ?? '';

    _titleController = TextEditingController(text: widget.data['title'] ?? '');
    _venueController = TextEditingController(text: widget.data['venue'] ?? '');
    _mapLocationController = TextEditingController(text: widget.data['mapLocation'] ?? '');
    _descriptionController = TextEditingController(text: widget.data['description'] ?? '');

    final dt = widget.data['dateTime'] as Timestamp?;
    if (dt != null) {
      _selectedDateTime = dt.toDate();
    }

    _initDynamicControllers(initialData: widget.data['categoryData'] ?? {});
  }

  void _initDynamicControllers({Map<String, dynamic>? initialData}) {
    ...
    for (var key in keys) {
      _dynamicControllers[key] = TextEditingController(
        text: initialData != null ? (initialData[key] ?? '') : '',
      );
    }

    if (initialData != null) {
      _rsvpEnabled = initialData['rsvpEnabled'] ?? false;
      _genderSpecificSessions = initialData['genderSpecificSessions'] ?? false;
      _occasionType = initialData['occasionType'] ?? 'Aqeeqa';
      _eventType = initialData['eventType'] ?? 'Sports';
    } else {
      _rsvpEnabled = false;
      _genderSpecificSessions = false;
      _occasionType = 'Aqeeqa';
      _eventType = 'Sports';
    }
  }
  ...
  Future<void> _submitForm() async {
    ...
    try {
      String coverImageUrl = _currentPhotoUrl;
      if (_selectedImage != null) {
        coverImageUrl = await CloudinaryService.uploadBillImage(_selectedImage!) ?? _currentPhotoUrl;
      }
      ...
      await FirebaseFirestore.instance.collection('invitations').doc(widget.docId).update(docData);

      if (mounted) {
        Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Invitation updated successfully!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error updating invitation: $e'),
                          ...
                          ClipRRect(
                            ...
                            child: Image.network(
                              ...
                              errorBuilder: (context, error, stackTrace) =>
                                  Icon(Icons.image, color: _primaryColor, size: 40),
                            ),
                          ),
                          Container(
                            decoration: BoxDecoration(
