...
class _RentalDetailScreenState extends State<RentalDetailScreen> {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      _buildBadge(_listing.type.replaceAll('_', ' ').toUpperCase(), _getTypeColor(_listing.type)),
                      _buildBadge(_listing.status.toUpperCase(), _listing.status == 'available' ? statusGreen : statusRed),
                    ],
                  ),
                  ...
                  // Main Specs Grid
                  const Text('Property Details', style: TextStyle(color: accentTeal, fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16),
                  GridView.count(
                    shrinkWrap: true,
                    ...
                    children: [
                      _buildSpecItem(Icons.king_bed, 'Rooms', _listing.rooms.toString()),
                      _buildSpecItem(Icons.bathtub, 'Bathrooms', _listing.bathrooms.toString()),
                      _buildSpecItem(Icons.chair, 'Furnishing', _listing.furnishingStatus.toUpperCase()),
                      _buildSpecItem(Icons.calendar_today, 'Available', DateFormat('dd MMM').format(_listing.availableFrom)),
                    ],
                  ),
                  const SizedBox(height: 24),
                  
                  // Amenities
                  const Text('Amenities', style: TextStyle(color: accentTeal, fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 12),
                  _buildAmenityRow(Icons.check_circle, 'Attached Bath', _listing.attachedBath),
                  _buildAmenityRow(Icons.check_circle, 'Gas Available', _listing.gasAvailable),
                  _buildAmenityRow(Icons.check_circle, 'Parking Available', _listing.parkingAvailable),
                  
                  const Divider(color: Colors.white10, height: 40),
                  
                  // Owner Info
                  const Text('Listed By', style: TextStyle(color: accentTeal, fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 12),
                  ListTile(
                    ...
                    subtitle: Text('Contact: ${_listing.ownerPhone}', style: const TextStyle(color: Colors.white70)),
                  ),
                ...
                ? SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: _isUpdating
                          ...
                          : const Text('Mark as Rented', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    ),
                  )
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  value,
                  style: const TextStyle(color: lightText, fontWeight: FontWeight.bold, fontSize: 12),
  ...
  Color _getTypeColor(String type) {
    switch (type) {
      ...
      case 'portion': return Colors.orange;
      case 'full_house': return Colors.green;
